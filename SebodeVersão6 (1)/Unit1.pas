...
var
  posVermelho_d12, posJapones_d12, posMaisBarato_d12: Integer;
  menorPreco_d12, i_d12: Integer;
begin
  ...
  posJapones_d12 := 0;
  posMaisBarato_d12 := 0;
  menorPreco_d12 := MaxInt;

  // Identificar posições importantes
  for i_d12 := 0 to 4 do
  begin
    ...
    if (Clientes[i_d12].Preco > 0) and (Clientes[i_d12].Preco < menorPreco_d12) then
    begin
      menorPreco_d12 := Clientes[i_d12].Preco;
      posMaisBarato_d12 := i_d12 + 1;
    end;
  end;

  // Verificação extra: estão na mesma posição
  if ((posVermelho_d12 > 0) and (posVermelho_d12 = posJapones_d12)) or
     ((posVermelho_d12 > 0) and (posVermelho_d12 = posMaisBarato_d12)) or
     ((posJapones_d12 > 0) and (posJapones_d12 = posMaisBarato_d12)) then
  begin
    LabelDica.Font.Color := clRed;
    Exit;
  end;

  // Se todos estão definidos
  ...
  // Se dois estão preenchidos, mas posição errada já é visível
  if (posMaisBarato_d12 > 0) and (posJapones_d12 > 0) and (posMaisBarato_d12 > posJapones_d12) then
  begin
    LabelDica.Font.Color := clRed;
    Exit;
  end;

  if (posVermelho_d12 > 0) and (posJapones_d12 > 0) and (posVermelho_d12 > posJapones_d12) then
  begin
    LabelDica.Font.Color := clRed;
    Exit;
  end;

  if (posVermelho_d12 > 0) and (posMaisBarato_d12 > 0) and (posVermelho_d12 < posMaisBarato_d12) then
  begin
    LabelDica.Font.Color := clRed;
    Exit;
  end;
